...
void Finder::parseFiles(std::string genefile, std::string fragfile, std::string samfile, std::string exprfile) {
	...
	Parser ps(genefile, fragfile, samfile, exprfile);
	ps.parseFiles(genes, frags, *sam, expressions); //, uniques, beds);
...
void Finder::findGeneExpression(Gene& g) {
	if(!expressions.empty()) {
		...
		//DAgo std::cerr << "ERROR - Expression file is empty." << std::endl;
...
void Finder::parFindConnections(std::vector<std::string> gvec, std::string dts,
	...
	while(!gqueue.empty() && level < sc_limit) {
		...
		// if(chunk == 0) chunk =1;
		// #pragma omp parallel for num_threads(nwrks) default(shared) schedule(dynamic,chunk)
		// for(unsigned i=0 s<conn_size; ++i) {
		// long thid = omp_get_thread_num();
