...
#include <windows.h>
#else
// Linux - enable large file support on 32 bit systems
...
// fstat64 is deprecated on MAC OS X (since 10.5)
#ifdef __APPLE__
...
MemoryMapped::MemoryMapped() : _filename   (),  _filesize   (0),
_hint       (Normal),  _mappedBytes(0),  _file       (0),
#ifdef _MSC_VER
_mappedFile (NULL),
#endif
_mappedView (NULL) { }
...
bool MemoryMapped::open(const std::string& filename, size_t mappedBytes, CacheHint hint) {
	...
	// initial mapping
	// first param is offset; zero means it is the first mapping
	remap(0, mappedBytes);
...
bool MemoryMapped::remap(uint64_t offset, size_t mappedBytes) {
	...
	if (offset > _filesize)
		return false;
	unsigned long physMem = getTotalRAM();
	if(mappedBytes >= physMem)
		return false;
...
unsigned long MemoryMapped::getTotalRAM() {
